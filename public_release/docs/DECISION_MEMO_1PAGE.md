...
**Decision thresholds (set pre-launch):** M2 logo lift vs control, M2 net proxy lift vs control, and improvement in net-minus-logo gap toward `>= 0 pp`  
**Guardrails:** margin proxy not worse than a finance-approved floor, no cohort-quality deterioration, no adverse credit-like / refund-like rate shift  
...
If controlled tests improve M2 logo retention by a pre-agreed percentage-point lift (`Delta`) among eligible new customers in the target families:
- Incremental repeat customers (M2) ~= `Eligible_New_Customers_in_Targets * Delta`
...
- Margin risk: repeat can rise while economics worsen if guardrails are not enforced.

## What we'd do next (2 weeks)
- **Week 1:** lock hypotheses + instrumentation, finalize numeric thresholds and guardrails with owners, assign accountability.
- **Week 2:** launch controlled tests per family; run interim readout; issue **scale / pause / iterate** decisions and set next M2 checkpoint dates.
